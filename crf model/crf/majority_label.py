...
# punctuations should not be in the top 10 list
def is_valid_word(word):
    return re.match(r'^[\w\-]+$', word) is not None and not re.match(r'^[\d\W_]+$', word)

# find top 10 most frequent words
def get_most_frequent_words(train_corpus):
    ...
    for sentence in train_corpus.sentences:
        ...
        for j in range(m):
            list.append((split_source[j], split_gloss[j])) 
        # all_morph_list.append(split_line[0])
    maj_counter = Counter(list)

    filtered = {
        (word, gloss): freq for (word, gloss), freq in maj_counter.items()
        if is_valid_word(word)
    }

    top = Counter(filtered).most_common(10) # change to 20 for top 20 most frequent words
    res = [pair for pair,_ in top]
    return res
...
#1. Change: add method to apply dictionary directly to corpus
def apply_majority_label_direct(majority_dictionary, corpus):
    new_prediction = []
    for sentence in corpus.sentences:
        sent_label_list = []
        for word in sentence.split_source:
            if word in majority_dictionary:
                sent_label_list.append(majority_dictionary[word])
            else:
                sent_label_list.append('UNK')
        new_prediction.append(sent_label_list)
    return new_prediction
