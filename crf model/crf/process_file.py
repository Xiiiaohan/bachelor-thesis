...
# 1. Change: take transcription instead of segmentation
#               add 't': 0 in field_dict
#               for case len = 3, change field_dict to 't' = 0, 'm' = 1, 'l' = 2
...
# 2. Change: check \t as beginning of sentence
            source = split_sentence[field_dict['t']].strip()
            utils.check_equality(source[0:3], r'\t ')
            ...
            if field_dict['l'] == -1: # If no translation
                ...
                utils.check_equality(translation[0:2], r'\l')
...
# 4. Change: replace Sentence class with CloseSentence class

            # print(source, gloss)
            self.sentences.append(CloseSentence(source[3:], gloss[3:], translation[3:], self.test))
    ...
    def convert_to_crf_format(self, stem=True, custom_dict=dict()):
        ...
        return [sentence.to_crf_format(stem=stem, custom_dict=custom_dict) for sentence in self.sentences]
    
# 3. Change: new class for covered files
...
class CloseSentence:
    def __init__(self, source, gloss, translation, test=False):
        ...
        self.split_source = self.source.split()
        self.split_gloss = self.gloss.split() if not test else [''] * len(self.split_source)
        self.n_words = len(self.split_source)

        if not self.test: utils.check_equality(self.n_words, len(self.split_gloss))

    #def to_crf_format(self):
    #    return list(zip(self.split_source, self.split_gloss))
    ...
    def to_crf_format(self, stem=False, custom_dict=dict()):
        ...
        for i in range(n_unit):
            source_word = self.split_source[i]

            if not self.test:
                gloss = self.split_gloss[i]
                cleaned = re.sub(r'[0-9\-.]', '', gloss)

                if cleaned.isupper():
                    pp_gloss = gloss
                elif custom_dict != dict() and \
                    (source_word in custom_dict and custom_dict[source_word] == gloss):
                    print('Custom dictionary', source_word, gloss)
                    pp_gloss = gloss
                else:
                    if stem:
                        pp_gloss = 'stem'
                ...
                pp_gloss = ''
            word_gloss_list.append((source_word, pp_gloss))
        return word_gloss_list    
...
class Sentence:
    ...
    def __init__(self, source, gloss, translation, test=False):
        ...
        if not self.test: utils.check_equality(self.n_morph, len(self.gloss.split()))
    ...
    def to_crf_format(self, stem=False, custom_dict=dict()):
        ...
        for i in range(n_unit): #self.n_morph):
            ...
            if not self.test:
                ...
                if gloss in ['-', '=']:
                    pp_gloss = gloss
                elif cleaned.isupper(): # Grammatical label (reference)
                    pp_gloss = gloss
                elif custom_dict != dict() and \
